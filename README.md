...
3\. 麦麦配置文件 `model\_config.toml` 中，`model\_task\_config.tool\_use` 项已配置完成，确保模型具备工具调用能力
...
重启麦麦主程序，插件会自动在 `plugins/maiplug\_message\_react` 目录下生成 `config.toml` 配置文件，如果没有请查看主程序加载插件的日志。
...
&nbsp;   # maiplug\_message\_react - 自动生成的配置文件
...
| 麦麦可回复消息，但不贴表情 | 更换具备工具调用能力的模型（推荐火山引擎 `doubao-seed-1-6-25061`），并校验 `model\_config.toml` 中 `tool\_use` 配置 |
