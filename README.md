...
| 插件正常运行，但麦麦不贴表情 | 更换具备工具调用能力的模型（一键包可浏览Web里的配置模板市场），并校验 `model_config.toml` 中 `tool_use` 配置 |
...
- 若插件运行异常，可查看麦麦日志文件定位具体问题，将报错内容复制发送给豆包、Gemini、Claude等推理大模型分析可以解决99%的问题